...
const page = () => {
  const userstep = (useSelector((state: RootState) => state.User.user?.step) ??
    1) as number;
  ...
  return (
    <div className="w-full min-w-[300px] mt-8 shadow-xl p-4">
      <div>
        <CurrStep currStep={userstep} />
      </div>
      <div className="">{renderStage()}</div>
      <div className="mt-8 flex justify-between gap-2">
        <div>
          {userstep > 2 ? (
            <Button variant={"indi"} size={"lg"} onClick={handlePrevious}>
              Previous
            </Button>
          ) : null}
        </div>
        <div>
          {userstep > 1 ? (
            <Button variant={"indi"} size={"lg"} onClick={handleNext}>
              {userstep < 5 ? `Save & Proceed` : `Save & Submit`}
            </Button>
          ) : null}
        </div>
      </div>
